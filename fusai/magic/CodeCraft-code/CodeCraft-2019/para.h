...
{
    const float para1 = 0.7;   //时间阻抗系数
    const float para2 = 1.4;            //时间阻抗指数
    ...
    float rati = (float)N_car/ (float)vol + 0.1; //考虑浮点数


    //计算时间阻抗
    //float R = 1 + para1 * pow(rati/para3,para2); // +
    float t  = 0;
    if(0.0 < rati && rati< 0.7)
    {
    	t = length / (base - base * rati * para2);
        std::cout << "t" << 1/(1-rati*para2) << std::endl;
        return t;
    }
    else
        return MYINF;
