...
protected:
    ...
    std::vector<std::vector<bool>> tabSta;
...
void TARDIS::_updateTARDIS(int const & Rb, int const & Re, int Tb, int const & Te)//节点Rb到Re， 对应的Tb和Te
{
    while(Tb < Te)
    {
        write(Tb,Rb,Re);
...
{
    std::ofstream outfile;
    outfile.open(std::string("id_from_to.txt"));
    ...
    for(int i = 0; i < eList.size(); i++)
    {
        ...
        int from = g.CrossId(eList[i].from);
        int to = g.CrossId(eList[i].to);
        sprintf(cstr,"%d,%d",from,to);
        adict.insert(std::pair<std::string,int>(std::string(cstr),j));
        //std::cout << std::string(cstr) << " , " << j << std::endl;
        outfile << eList[i].from << "," << eList[i].to << ", road " << g.roadid(from,to) << "," << j << std::endl;
        j=j+1;
        if(eList[i].isDuplex)
        {
            sprintf(cstr,"%d,%d",to,from);
            adict.insert(std::pair<std::string,int>(std::string(cstr),j));
            //std::cout << std::string(cstr) << " , " << j << std::endl;
            outfile << eList[i].to << "," << eList[i].from<< ", road " << g.roadid(to,from) << "," << j << std::endl;
            j++;
        }
    }
