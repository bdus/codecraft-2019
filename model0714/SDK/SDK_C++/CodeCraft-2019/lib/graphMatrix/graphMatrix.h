...
namespace MyDSA
{
    ...
    // 顶点的基本操作：查询第i个顶点（0 <= i < n）
        // void sn(int const &n) {this->n = n;}
        // void se(int const &e) {this->e = e;}
        virtual Tv& vertex ( int i ) { return V[i].data; } //数据
        virtual int inDegree ( int i ) { return V[i].inDegree; } //入度
        virtual int outDegree ( int i ) { return V[i].outDegree; } //出度
        virtual int firstNbr ( int i ) { return nextNbr ( i, this->nv ); } //首个邻接顶点
    ...
    // 边的确认操作
        ...
        virtual void insert ( Te const& edge, int w, int i, int j ) { //插入权重为w的边e = (i, j)
            ...
            this->ne++; V[i].outDegree++; V[j].inDegree++; //更新边计数与关联顶点的度数
        }
